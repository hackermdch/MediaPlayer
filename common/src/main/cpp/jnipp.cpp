...
namespace jni
{
	...
	jlong Object::getFieldValue(field_t field, internal::ReturnTypeWrapper<jlong> const&) const
	{
	...
	template <> void Object::set(field_t field, const jlong& value)
	{
	...
	static std::string detectJvmPath()
	{
		...
		{
			if (::RegQueryValueExA(versionKey, "CurrentVersion", NULL, NULL, buffer, &size) == ERROR_SUCCESS)
			{
				HKEY libKey;
				...
				if (::RegOpenKeyA(HKEY_LOCAL_MACHINE, keyName.c_str(), &libKey) == ERROR_SUCCESS)
				{
					...
					if (::RegQueryValueExA(libKey, "RuntimeLib", NULL, NULL, buffer, &size) == ERROR_SUCCESS)
					{
	...
	namespace internal
	{
		...
		void valueArg(value_t* v, jlong a) { ((jvalue*)v)->j = a; }
